...
data[data$mpg > 30] # Olvidarse de la coma despues de la condicion en filas

# Excluir columnas
data[1:5, c(-1, -9)] # Dame las filas 1 a 5 de todas las columnas menos a la 1 y 9
data[1:5, -c(1, 9)] # Otra forma (No valido para nombres)
data[1:5, !names(data) %in% c('No', 'car_name')]

data[data$mpg %in% c(15, 20), c('car_name', 'mpg')]

data[c(F, F, F, F, T), c(F, F, T)] # R "recicla" el vector de T/F que le das para la informacion a mostrar
