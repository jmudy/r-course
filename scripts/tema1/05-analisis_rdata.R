...
pedidos <- data.frame(clientes, fechas, pago) # Crear dataframe por columnas
...
load('../../data/tema1/pedidos.Rdata') # Cargar fichero .Rdata
orders <- readRDS('../../data/tema1/pedidos.rds') # Cargar fichero .rds
...
# Guardar varios objetos en un fichero .Rdata
primes <- c(2, 3, 5, 7, 11, 13)
pow2 <- c(2, 4, 8, 16, 32, 64, 128)
save(list = c('primes', 'pow2'), file = '../../data/tema1/primes_and_pow2.Rdata')

attach('../../data/tema1/primes_and_pow2.Rdata') # Cargar Rdata y ser notificado si ya existen dichos objetos
