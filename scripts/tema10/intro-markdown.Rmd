...
Este es un fichero creado por **Jesús Mudarra** <https://github.com/jmudy>
...
Después de cargar los datos, solemos mirar información acerca de los primeros elementos del dataframe, la estructura de las columnas del mismo o incuso un resumen de los estadísticos básicos de cada una.

```{r}
htmlTable(head(auto),
          caption = 'Tabla con las primeras entradas del dataset auto',
...
kable(head(auto), 'html') %>%
  kable_styling(bootstrap_options = c('stiped',
                                      'hover',
                                      'condensed'),
...
Hemos hecho uso de un dataframe de cochese que contiene `r nrow(auto)` muestras de coches y cada uno de ellos tienen `r ncol(auto)` variables.
