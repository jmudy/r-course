...
library(ROCR) # Instalar con install.packages('ROCR')
...
plot(perf1)
lines(par()$usr[1:2], par()$usr[3:4]) # Dibujar línea diagonal

prob_cuts_1 <- data.frame(cut = perf1@alpha.values[[1]],
...
prob_cuts_1[prob_cuts_1$tpr >= 0.8, ]
