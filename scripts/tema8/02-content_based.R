...
movie_title_df <- unique(movie_title_df) # Eliminar películas duplicadas
...
getRecommendations <- function(movie_df, user_df, userID){
  ...
  recommendation <- getRecommendedMovies(like, mclust, movie_df)
  # Eliminar las películas que el usuario ya ha visto
  recommendation <- recommendation[-activeUser$MovieID]
  # Añadimos el título de la película
  movieTitle <- movie_df[match(recommendation, movie_df$MovieID), 2]
  recommendation <- data.frame(recommendation, movieTitle)
...
users_df[users_df$UserID == 196, ]$MovieID
